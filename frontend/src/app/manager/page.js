...
export default function ManagerDashboard() {
  ...
  const handleAddFeedback = async (goalId) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Feedback saved to the employee's check-in record!");
      } else {
        alert("Wait for the employee to submit their first check-in before adding feedback.");
      }
    } catch (error) {
  ...
  const handlePushSharedGoal = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchGoals(); // Refresh table to see the newly pushed goals
        alert("Department goal successfully pushed to all employees!");
      }
    } catch (error) {
  ...
  if (loading) return <div className="p-8">Loading team goals...</div>;
          ...
          <Dialog open={isPushModalOpen} onOpenChange={setIsPushModalOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <form onSubmit={handlePushSharedGoal} className="space-y-4 pt-4">
                ...
                <Button type="submit" className="w-full bg-zinc-900">Push to All Employees</Button>
              </form>
            </DialogContent>
          </Dialog>
        </div>
        ...
        <div className="bg-white rounded-md border shadow-sm overflow-hidden">
          <Table>
            ...
            <TableBody>
              {goals.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center h-24 text-zinc-500">No goals submitted yet.</TableCell>
                </TableRow>
              ) : (
                goals.map((goal) => (
                    ...
                    <TableCell className="text-right">
                      {goal.locked ? (
                        <div className="flex justify-end gap-2">
                          <Button size="sm" variant="outline" className="border-blue-200 text-blue-700" onClick={() => handleAddFeedback(goal.id)}>
                            💬 Add Feedback
                          </Button>
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <Button size="sm" className="bg-zinc-900" onClick={() => handleApprove(goal.id)}>Approve</Button>
